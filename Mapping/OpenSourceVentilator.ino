...
boolean checkValues()
{
  ...
  if (reqVolume > maxVolume)
    isOk = false;
  if (reqCompression < minCompression)
    isOk = false; // Compression in allowed range ?
  if (reqCompression > maxCompression)
    isOk = false;
  if (syncRatio < minSyncRatio)
    isOk = false; // portion of the cycle for breath synchronisation in allowed range ?
  if (syncRatio > maxSyncRatio)
    isOk = false;
  if (expirationRatio < minExpirationRatio)
    isOk = false; // portion of the cycle for breath synchronisation in allowed range ?
  if (expirationRatio > maxExpirationRatio)
    isOk = false;
  if (isnan(reqBPM))
    isOk = false; // Check for malformed floating point values (NaN)
  if (isnan(reqVolume))
    isOk = false;
  if (isnan(reqCompression))
    isOk = false;
  if (isnan(syncRatio))
    isOk = false;
  if (isnan(expirationRatio))
    isOk = false;
  return isOk;
}
...
void doDisplay()
{
  ...
  while (disp[f])
    if (disp[f++] == '.')
      c++;     // Calculates the stringt length without the dot characters
...
void eeput(int n) // records to EEPROM (only if values are validated)
{
  ...
  if (n == 1)
    isOk = true; // override (for debug testing)
  if (isOk)
  {
...
void eeget()
{
  ...
  boolean isOk = checkValues();
  if (tac > 1)
    isOk = false; // is the value 0 or 1 ?
  if (cvm > 1)
    isOk = false; // is the value 0 or 1 ?
  if (!isOk)
  {
  ...
  CVmode = (cvm > 0);
  if (term)
    Serial.print((isOk) ? F("Read EEPROM Settings\n") : F("Read Default Settings\n"));
#else
  reqBPM = defaultBPM;
  reqVolume = defaultVolume;
  reqCompression = defaultCompression;
...
void SerialCommand()
{
  ...
  case 'S': // S set Speed (in BPM)
    ...
    if ((r >= minBPM) && (r <= maxBPM))
    {
      ...
      Serial.println(F("Out of range (6-40BPM)"));
    break;
  case 'V': // V set Volume (in liters)
    ...
    if ((r >= minVolume) && (r <= maxVolume))
    {
      ...
      Serial.println(F("Out of range (0.1-1.5l)"));
    break;
  case 'P': // P Set pressure (in KPa)
    ...
    if ((r >= minCompression) && (r <= maxCompression))
    {
      ...
      Serial.println(F("Out of range (1-20KPa)"));
    break;
  case 'E': // E set expiration ratio (x inspiration)
    ...
    if ((r >= minExpirationRatio) && (r <= maxExpirationRatio))
    {
      ...
      Serial.println(F("Out of range (1 to 3 x inspiration)"));
    break;
  case 'Y': // Y set sYnc (in % of cycle)
    ...
    if ((r >= minSyncRatio) && (r <= maxSyncRatio))
    {
      ...
      Serial.println(F("Out of range (0-40%)"));
    break;
  ...
  while (t--)
  {
    ...
    while (Serial.available())
    {
      ...
      if (inChar != '\r')
      {
        ...
        if (BufferLength >= CommandMaxLength)
        {
          BufferLength = 0;
          Buffer = "";
...
void Timer()
{
  ...
  else
  {
    ...
    switch (timCnt % 50)
    { // 10 ms tick period for interface handling
    case 0:
      if (--dispCnt <= 0)
        displayMenu();
      break;
...
void detectCurrentSensor() // detects if a Allegromicro current sensor is connected to pin_current_Sense or not.
{
  ...
  if (sensCurrent)
    Serial.println(F("Found current sensor!"));
}
#endif
...
void setup()
{
  ...
  int c = 1000;
  while (!Serial && c--)
    delay(1); // wait for serial port to connect. Needed for native USB (non blocking)
  term = Serial;
  if (term)
    Serial.println(F("Open Source Ventilator Ver 0.14"));
  Wire.begin();
  detectBMEsensor();
#ifdef CurrentSense
  detectCurrentSensor()
...
void pressPrevNext()
{
  if ((keys[btnPrev]) && (menuItem > 0))
  {
    menuItem--;
    setDisplayMenu(Menu_message + 10 * menuItem, std_dly, 0);
  } // Previous menu item (no rollover)
  if ((keys[btnNext]) && (menuItem < MenuItems - 1))
  {
    menuItem++;
    setDisplayMenu(Menu_message + 10 * menuItem, std_dly, 0);
  } // Next menu item (no rollover)
}
...
void pressUpDown()
{
  ...
  case 0:
    if (n < 0)
      dispParam = (dispParam == 0) ? 1 : 0; // button Dn triggers extended data
    if (n > 0)
      CVmode = !CVmode; // button Up triggers control mode change
    if (dispParam)
      setDisplayMenu(Measurement_Message);
    if (n == 1)
      setDisplayMenu(Measurement_Start);
    dispCnt = 0;
    break;
  case 1:
...
void loop()
{
  ...
  if (millis() > tick + samplePeriod)
  {
    ...
    {                                                             // CP Mode - bargraph needs to show volume
      if ((relPressure > compressionScale) && (breathPhase == 1)) // Check for pressure
      {
        ...
        if (stepper.currentPosition() < (volume * motorVolumeRatio) * failVolumeRatio)
          beep(300); // Insufficient volume alarm
      }
      bargraphDot = 1;
    ...
    else if (highPressureAlarmCnt)
      highPressureAlarmCnt--;
    // Keyboard actions
    if (keys[btnOk] && !mkeys[btnOk])
      pressOk(); // Ok button
    if (keys[btnPrev] && !mkeys[btnPrev])
      pressPrevNext(); // previous item button
    if (keys[btnNext] && !mkeys[btnNext])
      pressPrevNext(); // next item button
    if (keys[btnDn] && !mkeys[btnDn])
      pressUpDown(); // lower value button
    if (keys[btnUp] && !mkeys[btnUp])
      pressUpDown(); // increase value button
    byte ct = maxBtn;
    while (ct--)
    {
      if (keys[ct])
  ...
  if ((millis() > breathe + breathLength) && (stepper.currentPosition() > 0))
    breathe += 50;                       // wait for the previous cycle to finish (safety)
  if (millis() > breathe + breathLength) // start of a cycle
  {
    if (!allSet)
      sweepValues(); // Manage the ramp up / ramp down of parameters
    breathLength = int(60000 / BPM);
